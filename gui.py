...
def thread_it(func, *args):
    ...
    t.start()
    # 阻塞--卡死界面！
filep = './xun1/ScenarioFolder/' #"训练集场景信息文件位置"
fileq = './test1/ScenarioFolder/' #"测试集场景信息文件位置"
def count1(filepath):
    ...
    for s in pathDir:
        ...
        if os.path.isfile(newDir) :         #如果是文件
            if os.path.splitext(newDir)[1]==".txt":  #判断是否是txt
                #sceneFileName, threatFileName = readFile(n ewDir)
                if 'B' in newDir:
...
bt1 = tkinter.Button(frame,image=p1,text="训练",compound = "left",command=lambda:thread_it(show5))
bt1.pack(side="left") 
bt2 = tkinter.Button(frame,text="航迹规划",image=p2,compound = "left",command=lambda:thread_it(show6))
bt2.pack(side="left",) 
bt3 = tkinter.Button(frame,text="停止",image=p3,compound = "left",command=lambda:thread_it(os._exit(0)))
bt3.pack(side="left",) 
bt4 = tkinter.Button(frame,text="训练文件",image=p4,compound = "left",command=lambda:thread_it(show1))
bt4.pack(side="left",) 
bt5 = tkinter.Button(frame,text="测试文件",image=p4,compound = "left",command=lambda:thread_it(show2))
bt5.pack(side="left",) 
bt6 = tkinter.Button(frame,text="准确率",image=p5,compound = "left",command=lambda:thread_it(show3))
bt6.pack(side="left",) 
bt7 = tkinter.Button(frame,text="航迹规划结果",image=p6,compound = "left",command=lambda:thread_it(show4))
bt7.pack(side="left",) 
